...
class Validations:
    ...
    def create_incident_validate(self, contentType, data):
        ...
        if "incident_type" not in data or "title" not in data or "created_by" not in data or "location" not in data or "status" not in data or "comment" not in data:
            return jsonify({
                "status": 400,
                "message": "Wrong body format"
            })
        ...
        if not isinstance(incident_type, str) and not isinstance(title, str) and not isinstance(created_by, str)  and not isinstance(location, str) and not isinstance(status, str) and not isinstance(comment, str):
            return jsonify({
                "status": 400,
                "message":"Fields must be Strings"
            })

        if not incident_type and not title and not created_by and not location and not status and not comment:
            return jsonify({
                "status": 400,
