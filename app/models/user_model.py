...
class User:
    ...
    def save(self):
        ...
        query = """
              ...
              VALUES('{}', '{}', '{}', '{}', '{}')""".format(
                self.username, self.password, self.email, self.phone_number,
                 self.is_admin)

        cursor.execute(query)
        DatabaseConnection.commit()

    # do a query to check if our user exists in the database by
    # using the username and the email
    @staticmethod
    def check_user(username, email):
        cursor = DatabaseConnection.cursor()
        # here we use subquries because we dont want to return some data
        #  from the database like the password
        query = """SELECT row_to_json(result) FROM (SELECT user_id, username,
        email, phone_number, is_admin FROM users) result WHERE
        username = '{}' OR email = '{}';""".format(username, email)
        cursor.execute(query)
    ...
    def login_user(username, password):
        cursor = DatabaseConnection.cursor()
        query = "SELECT row_to_json(users) FROM users WHERE username = %s;"
        cursor.execute(query, (username,))
        # returns the first record from the database that matches
        ...
        if user_login and check_password_hash(hashed_password, password):
            ...
            # remove password and user_id from the response
            del user_login[0]['password']
            del user_login[0]['user_id']
    ...
    # """Decoding the token to get the payload and then
    #  return the user Id in 'sub'
    ...
    def decode_auth_token(token):
        try:
            ...
            # All our expired or invaild tokens will be stored in 
            # blacklist_token table so that they are not used again.
            # To do this we use a class and a method to actually check
...
class BlacklistToken:
    ...
    def blacklist(self):
        ...
        """
        cursor = DatabaseConnection.cursor()
        query = """INSERT INTO blacklist_token(token,
                  blacklisted_on) VALUES('{}', '{}')""".format(
